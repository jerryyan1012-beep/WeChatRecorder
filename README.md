...
使用 **双流音频捕获** 技术同时录制双方声音：

1. **系统声音** - 使用 WASAPI Loopback 捕获扬声器/耳机输出（对方声音）
2. **麦克风输入** - 直接捕获麦克风输入（我方声音）
3. **混合输出** - 合并为双声道立体声 WAV 文件

**声道分配**：
- 左声道 (Left): 对方声音 (系统输出)
- 右声道 (Right): 我方声音 (麦克风输入)
...
### v1.0.2 (2024-05-08) - 双方声音录音修复
- 修复只能录制一方声音的问题
- 现在同时录制麦克风和系统输出
- 输出双声道 WAV: 左声道=对方, 右声道=我方
- 修复连续录制多次后闪退的问题
- 改进内存管理和线程清理
